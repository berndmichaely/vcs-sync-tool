...
public class SynchronizeDirWithVcsWorkingCopy
{
	...
	private interface SystemCommand
	{
		...
		/**
		 * Handle unchanged file, which is present in src and unmodified in dst.
		 *
		 * @param subPath the relative file path to change
		 * @throws ScriptException if an {@link IOException} occurs or running a
		 *                         system command fails
		 */
		void unchangedFile(Path subPath);
	...
	{
		private void logPath(Op op, Path path, AnsiColorEscapeCodes color)
		{
			final String prefix = String.format("%5s : ", op);
		...
		{
			logPath(Op.MOD, subPath, AnsiColorEscapeCodes.YELLOW);
		}

		@Override
		public void unchangedFile(Path subPath)
		{
			if (beVerbose)
			{
				logPath(Op.UNMOD, subPath, AnsiColorEscapeCodes.BLUE);
			}
		}
	...
	/**
	 * Tests two files for content equality. Files are assumed to exist.
	 *
	 * @param file1 first file to compare
	 * @param file2 second file to compare
	 * @return true, iff bothe files are equal
	 */
	private static boolean filesEqual(Path file1, Path file2)
	{
		try (final var i1 = new BufferedInputStream(Files.newInputStream(file1)))
		{
			try (final var i2 = new BufferedInputStream(Files.newInputStream(file2)))
			{
				for (;;)
				{
					final int byte1 = i1.read();
					final int byte2 = i2.read();
					final boolean hasNext1 = byte1 >= 0;
					final boolean hasNext2 = byte2 >= 0;
					if (!hasNext1 && !hasNext2)
					{
						return true;
					}
					else if (hasNext1 != hasNext2 || byte1 != byte2)
					{
						return false;
					}
				}
		...
		{
			throw new ScriptException(24, ex);
		}
	}
	...
	{
		List.of(
			"usage : " + SynchronizeDirWithVcsWorkingCopy.class.getSimpleName() + " [ -v ] < -d | -s | -g > <source-directory> <destination-directory>",
	...
	private class PathAction implements Comparable<PathAction>
	{
		...
		private void run()
		{
			systemCommands.forEach(systemCommand ->
			{
				switch (operation)
				{
					case MKDIR -> systemCommand.addDir(path);
					case ADD -> systemCommand.addFile(path);
					case MOD -> systemCommand.changeFile(path);
					case UNMOD -> systemCommand.unchangedFile(path);
					case DEL -> systemCommand.removeFile(path);
					case RMDIR -> systemCommand.removeDir(path);
					default -> throw new ScriptException(98, "Invalid Op: " + operation);
				}
			});
		}
	}
	...
	private void applyChanges()
	{
		Stream.of(
			...
			srcFiles.stream().filter(dstFiles::contains).map(file -> new PathAction(file,
				filesEqual(baseDirSrc.resolve(file), baseDirDst.resolve(file)) ? Op.UNMOD : Op.MOD)),
